/* ─── HELP / USER GUIDE CONTENT (EN / PL / RU) ───
   ...
   Structure: HELP_CONTENT[lang][section], where section is one of
   'interface' | 'process' | 'collaboration' — three tabs shown inside the
   dialog (see renderHelpDialogHtml()/setHelpSection() in app.js), on top
   of the existing EN/PL/RU language switcher. 'interface' covers the
   editor chrome itself (files, navigation, visual options, shortcuts);
   'process' covers process-diagram-specific authoring (Call Activities,
   element properties, dictionaries); 'collaboration' covers Collaboration
   diagrams — pools/participants, Choreography Task, and message
   envelopes. */
const HELP_CONTENT = {
  en: {
    interface: `<h3>Getting started</h3>
...
<p>The <b>Extended</b> toggle in the toolbar overlays extra information directly on the canvas: every dictionary set to something other than "Hide" (see "Dictionaries (tags)" in the Process diagrams tab) draws a colored badge at its chosen corner of the element, elements with a Details link get a small ↗ icon you can click straight from the diagram, and elements with more than one visible dictionary badge on the same corner stack in "Sort" order rather than overlapping. It's off by default to keep the canvas clean — switch it on when you need an at-a-glance overview.</p>
...
`,
    process: `<h3>Call Activities: linking sub-processes together</h3>
...
<p>If <b>Details</b> holds a web address, a small link icon appears on the shape (see "Extended details view" in the Interface tab) so anyone opening the diagram can jump straight to that page.</p>
...
`,
    collaboration: `<h3>Pools and participants</h3>
<p>Use <b>Create pool/participant</b> from the palette to start a Collaboration diagram — a pool represents a participant (an organization, system, or role) and, like everywhere else in BPMN, can be split into lanes. Pool and lane backgrounds are rendered fully transparent so the canvas grid stays visible through them.</p>

<h3>Choreography Task</h3>
<p>The <b>Choreography Task</b> tool — in its own "Choreografia" section of the palette — drops a task-like shape split into three bands: <b>Header</b>, <b>Body</b>, and <b>Footer</b>. Double-click any band to edit its text directly: press <b>Enter</b> to confirm, <b>Shift+Enter</b> for a line break within the same band, or <b>Escape</b> to cancel without saving. Select part of the text and press <b>Ctrl/Cmd+B</b> before confirming to make it bold — exactly like editing a regular Task's label.</p>
<p>Selecting a Choreography Task adds a <b>"Swap header/footer colors"</b> icon to its context pad, so the two bands can trade fill colors without retyping anything.</p>

<h3>Message envelopes</h3>
<p>Select a Choreography Task to reveal a small <b>+</b> button above and below it — one per zone; once a zone already has an envelope, its button disappears until that envelope is removed. Click it to attach a message envelope, connected to the task with a dotted line.</p>
<p>Double-click an envelope's icon to give it a caption, using the same Enter-to-confirm / Shift+Enter-for-a-line-break / Escape-to-cancel and Ctrl/Cmd+B-to-bold behavior as a choreography band. The envelope icon's own size and proportions never change, no matter how long the caption is or whether it wraps onto more lines.</p>
<p>Selecting an envelope adds a context-pad icon that moves its caption between <b>below</b> the icon (the default — meaning above the icon for a header envelope and below it for a footer one, always opening onto empty canvas rather than squeezing against the task) and <b>beside</b> it, vertically centered either way.</p>
`
  },
  pl: {
    interface: `<h3>Pierwsze kroki</h3>
...
`,
    process: `<h3>Call Activities: łączenie subprocesów</h3>
...
<p>Jeśli pole <b>Details</b> zawiera adres strony, na kształcie pojawia się mała ikona linku (patrz „Rozszerzony widok szczegółów" w zakładce Interfejs) — każdy otwierający diagram może od razu przejść na tę stronę.</p>
...
`,
    collaboration: `<h3>Pule i uczestnicy</h3>
<p>Użyj narzędzia <b>Create pool/participant</b> z palety, żeby zacząć diagram Collaboration — pula reprezentuje uczestnika (organizację, system albo rolę) i, tak jak wszędzie w BPMN, można ją podzielić na tory. Tła pul i torów są w pełni przezroczyste, dzięki czemu siatka płótna jest przez nie cały czas widoczna.</p>

<h3>Choreography Task</h3>
<p>Narzędzie <b>Choreography Task</b> — we własnej sekcji "Choreografia" w palecie — tworzy kształt przypominający zadanie, podzielony na trzy pasy: <b>Header</b>, <b>Body</b> i <b>Footer</b>. Kliknij dwukrotnie dowolny pas, żeby edytować jego tekst bezpośrednio: <b>Enter</b> zatwierdza, <b>Shift+Enter</b> dodaje nową linię w tym samym pasie, a <b>Escape</b> anuluje bez zapisu. Zaznacz fragment tekstu i naciśnij <b>Ctrl/Cmd+B</b> przed zatwierdzeniem, żeby go pogrubić — dokładnie tak jak przy edycji etykiety zwykłego Task.</p>
<p>Zaznaczenie Choreography Task dodaje do jego panelu kontekstowego ikonkę <b>"Swap header/footer colors"</b>, która zamienia kolory wypełnienia obu pasów miejscami bez przepisywania czegokolwiek.</p>

<h3>Koperty wiadomości</h3>
<p>Zaznacz Choreography Task, żeby zobaczyć mały przycisk <b>+</b> nad nim i pod nim — jeden na strefę; gdy strefa ma już kopertę, jej przycisk znika, dopóki ta koperta nie zostanie usunięta. Kliknięcie dodaje kopertę wiadomości, połączoną z zadaniem przerywaną linią.</p>
<p>Kliknij dwukrotnie ikonkę koperty, żeby nadać jej podpis — obowiązuje to samo zachowanie Enter-zatwierdza / Shift+Enter-nowa-linia / Escape-anuluje oraz Ctrl/Cmd+B-pogrubienie, co w pasach choreografii. Rozmiar i proporcje samej ikonki koperty nigdy się nie zmieniają, niezależnie od długości podpisu ani tego, czy zawinie się on na kolejne linie.</p>
<p>Zaznaczenie koperty dodaje ikonkę w panelu kontekstowym, która przenosi jej podpis między położeniem <b>pod</b> ikonką (domyślne — czyli nad ikonką dla koperty w Header, pod nią dla koperty w Footer, zawsze otwierając się na puste płótno zamiast ściskać się przy zadaniu) a położeniem <b>obok</b> niej, w obu przypadkach wyśrodkowanym w pionie.</p>
`
  },
  ru: {
    interface: `<h3>Начало работы</h3>
...
<p>Переключатель <b>Extended</b> в панели инструментов накладывает дополнительную информацию прямо на холст: каждый словарь, для которого задано что-то, кроме "Hide" (см. «Словари (метки)» во вкладке Диаграммы процессов), рисует цветной значок в выбранном углу элемента, элементы со ссылкой в поле Details получают маленький кликабельный значок ↗ прямо на диаграмме, а элементы с несколькими видимыми значками словарей в одном и том же углу выстраиваются по порядку "Sort" вместо наложения друг на друга. По умолчанию режим выключен, чтобы не загромождать холст — включайте его, когда нужен быстрый обзор сразу всей информации.</p>
...
`,
    process: `<h3>Call Activities: связывание подпроцессов</h3>
...
<p>Если в поле <b>Details</b> указан веб-адрес, на фигуре появляется небольшой значок ссылки (см. «Расширенный режим отображения деталей» во вкладке Интерфейс) — любой, кто откроет диаграмму, сможет сразу перейти на эту страницу.</p>
...
`,
    collaboration: `<h3>Пулы и участники</h3>
<p>Используйте инструмент <b>Create pool/participant</b> на палитре, чтобы начать диаграмму Collaboration — пул представляет участника (организацию, систему или роль) и, как и везде в BPMN, может быть разделён на дорожки. Фон пулов и дорожек полностью прозрачен, поэтому сетка холста всегда видна сквозь них.</p>

<h3>Choreography Task</h3>
<p>Инструмент <b>Choreography Task</b> — в собственном разделе "Choreografia" на палитре — создаёт фигуру, похожую на задачу, разделённую на три полосы: <b>Header</b>, <b>Body</b> и <b>Footer</b>. Дважды щёлкните по любой полосе, чтобы редактировать её текст напрямую: <b>Enter</b> подтверждает, <b>Shift+Enter</b> добавляет перенос строки в той же полосе, а <b>Escape</b> отменяет без сохранения. Выделите часть текста и нажмите <b>Ctrl/Cmd+B</b> перед подтверждением, чтобы сделать её жирной — точно так же, как при редактировании подписи обычного Task.</p>
<p>При выделении Choreography Task в панель контекстных действий добавляется значок <b>"Swap header/footer colors"</b>, который меняет местами цвета заливки обеих полос без необходимости что-либо перепечатывать.</p>

<h3>Конверты сообщений</h3>
<p>Выделите Choreography Task, чтобы увидеть небольшую кнопку <b>+</b> сверху и снизу от неё — по одной на зону; как только в зоне уже есть конверт, её кнопка исчезает, пока этот конверт не будет удалён. Клик добавляет конверт сообщения, соединённый с задачей пунктирной линией.</p>
<p>Дважды щёлкните по значку конверта, чтобы задать его подпись — действуют те же правила Enter-подтверждает / Shift+Enter-перенос-строки / Escape-отменяет и Ctrl/Cmd+B-полужирный, что и в полосах хореографии. Размер и пропорции самого значка конверта никогда не меняются, независимо от длины подписи и от того, переносится ли она на несколько строк.</p>
<p>При выделении конверта появляется значок в панели контекстных действий, который переключает положение подписи между <b>под</b> значком (по умолчанию — над значком для конверта в Header, под ним для конверта в Footer, всегда открываясь на пустой холст, а не прижимаясь к задаче) и <b>рядом</b> с ним, в обоих случаях по центру по вертикали.</p>
`
  }
};
